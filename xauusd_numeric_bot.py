...
# ============================================================
REMINDER_EVENTS = [
    {"date": "2026-08-19", "label": "Минутки FOMC — протокол заседания 28-29 июля"},
    {"date": "2026-08-27", "label": "Джексон-Хоул — симпозиум ФРС, возможные сигналы от Уорша"},
    {"date": "2026-09-04", "label": "NFP за август (BLS)"},
    {"date": "2026-09-11", "label": "CPI за август (BLS)"},
    {"date": "2026-09-15", "label": "FOMC — решение по ставке, день 1"},
    {"date": "2026-09-16", "label": "FOMC — решение + пресс-конференция"},
]
...
def classify_nfp(value):
    ...
    ИСПРАВЛЕНО: было захардкожено "+", из-за чего отрицательные значения
    (потеря рабочих мест) отображались как "+-23К" — теперь используется
    формат {value:+}, который сам ставит правильный знак (+ для
    положительных, − для отрицательных)."""
    if value is None:
        return None
    if value < 100:
