...
def fetch_nfp_change():
    ...
    ИСПРАВЛЕНО ВТОРОЙ РАЗ, теперь на основе реального теста (09.07.2026):
    первое предположение (что BLS отдаёт серию уже в тысячах человек) было
    неверным — эмпирически на живом запуске разница латест-прев дала ровно
    57000, что совпадает с реальным известным значением "+57K рабочих мест"
    БЕЗ какого-либо умножения. Значит BLS в этой серии отдаёт абсолютные
    единицы (реальных людей), а не тысячи. Чтобы получить привычный формат
    отчётов NFP ("+57K"), нужно делить на 1000, а не умножать."""
